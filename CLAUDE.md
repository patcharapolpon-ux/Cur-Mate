# CLAUDE.md

ไฟล์นี้ให้คำแนะนำแก่ Claude Code (claude.ai/code) เมื่อทำงานกับโค้ด/เอกสารใน repository นี้

## จุดประสงค์ของ repository นี้

repository นี้เป็น **vault เอกสาร (Obsidian)** ของโปรเจกต์ "Curmate" — ไม่มีซอร์สโค้ดของแอปพลิเคชัน ไม่มี build script และไม่มี test ใดๆ จึงไม่มีคำสั่ง build / lint / run การทำงานใน repo นี้คือการอ่าน เขียน และจัดระเบียบโน้ต Markdown เท่านั้น

**ข้อยกเว้น**: โฟลเดอร์ `homework/` เก็บโค้ดแบบฝึกหัดของคอร์สที่เรียนอยู่ (เช่น `homework/curmate-mvp/` เป็น static HTML/CSS/JS + Firebase สำหรับการบ้านเรื่อง database) ไม่ใช่ส่วนหนึ่งของ pipeline เอกสารด้านล่าง และไม่ใช่แอปพลิเคชัน Curmate จริง — อยู่แยกจาก `docs/` โดยเจตนา ไม่ต้องพยายามเชื่อมโยงเข้ากับ pipeline หรือให้ agent/skill ของ vault เข้าไปยุ่งเกี่ยว

## โครงสร้างและลำดับการไหลของเอกสาร

เอกสารทั้งหมดอยู่ใต้ `docs/` จัดเรียงเป็น pipeline ที่มีลำดับตัวเลข โดยแต่ละขั้นตอนจะส่งต่อไปยังขั้นถัดไป:

```
01-requirements/backlog.md → รายการสรุป requirement ดิบทั้งหมดแบบตาราง (จุดเริ่มก่อนแตกเป็น spec)
01-requirements/01-spec    → สิ่งที่ระบบต้องมี (ต้นทาง/source of truth)
01-requirements/02-plan    → roadmap/phase (feature-list, release-plan) ที่แตกมาจาก spec
01-requirements/03-task    → งานย่อยที่ลงมือทำได้จริง แตกมาจาก plan
02-design/01-prototypes    → user journey (Mermaid) + mockup UI/UX (HTML) อ้างอิงจาก 01-spec/02-plan ตาม design system กลางใน DESIGN.md
02-design/02-technical     → ออกแบบเชิงเทคนิค (architecture/DB/API) อ้างอิงจาก 01-prototypes
03-testing/01-test-plan    → acceptance criteria/test plan/test case อ้างอิงจาก 02-technical และ 01-spec
03-testing/02-test-result  → ผลทดสอบ pass/fail และบั๊ก อ้างอิงจาก 01-test-plan
04-retrospectives          → สรุปบทเรียน อ้างอิงจาก 02-test-result และ 05-log
05-log                     → บันทึกความเคลื่อนไหว/การตัดสินใจตามลำดับเวลา
00-archived                → เอกสารที่เลิกใช้แล้ว/ถูกยกเลิก (ดูกฎด้านล่าง)
```

แต่ละโฟลเดอร์มีไฟล์ `index.md` อธิบายจุดประสงค์ของตัวเอง และเชื่อมโยงไปมาตาม pipeline นี้ด้วย Obsidian wikilink (`[[../relative/path/index|label]]`) เวลาจะเพิ่มเอกสารใหม่ ให้วางในโฟลเดอร์ที่ตรงกับขั้นตอนนั้นๆ และลิงก์เข้ากับ chain ที่มีอยู่ อย่าปล่อยให้เอกสารลอยตัวไม่มีการเชื่อมโยง

## ภาษาที่ใช้ตอบแชท

เมื่อทำงานใน repository นี้ ให้**ตอบข้อความแชท (สื่อสารกับผู้ใช้) เป็นภาษาไทยเสมอ** ไม่ใช่แค่ตอนที่ถูกขอ — ครอบคลุมคำอธิบาย สรุปผลงาน และคำถามที่ถามผู้ใช้ ส่วนเนื้อหาที่เป็นข้อกำหนดทางเทคนิคจริง เช่น ชื่อไฟล์, path, syntax, ชื่อฟิลด์ (Given/When/Then, MoSCoW) ยังคงเป็นภาษาอังกฤษได้ตามปกติ

## ข้อตกลง/ธรรมเนียมของโปรเจกต์

- เนื้อหาเอกสารทั้งหมดเขียนเป็น**ภาษาไทย**
- **ห้ามลบ**เอกสาร/เนื้อหาที่ไม่ใช้แล้วโดยตรง แบ่งเป็น 2 กรณีตามลักษณะเอกสาร:
  - **เอกสารทั้งไฟล์ที่ถูกแทนที่ทั้งหมด** (เช่น user journey เดิมที่เลิกใช้ทั้ง flow) — ย้ายทั้งไฟล์ไปเก็บที่ `docs/00-archived/` แทน
  - **living document ที่มีหลายรายการอยู่ในไฟล์เดียว** (เช่น `backlog.md`, `feature-list.md`, `acceptance-criteria.md`, `test-cases/*.md`) — **ห้ามย้ายทั้งไฟล์และห้ามลบแถว/หัวข้อเดิมออก** ให้ทำเครื่องหมายสถานะ/ล้าสมัยไว้ในตำแหน่งเดิมแทน (เช่น เปลี่ยนสถานะเป็น `Won't have (this time)`, เติมข้อความ "(ล้าสมัย — เหตุผล...)") เพื่อรักษาประวัติการตัดสินใจไว้โดยไม่ทำให้ลำดับ/เลขอ้างอิงของรายการอื่นเปลี่ยน
  
  ทั้งสองกรณีมีเป้าหมายเดียวกันคือรักษาประวัติการตัดสินใจไว้เสมอ ไม่ลบทิ้งเฉยๆ
- การอ้างอิงข้ามเอกสารใช้ Obsidian wikilink แบบ relative path (`[[../02-design/index|02-design]]`) ไม่ใช่ Markdown link ธรรมดา — ให้เขียนลิงก์ในรูปแบบเดียวกันนี้เสมอ
- `.obsidian/` เก็บ state ของ UI ตัว vault เอง (ไม่ใช่เนื้อหาโปรเจกต์) และ `.obsidian/workspace.json` ถูกใส่ไว้ใน `.gitignore` เพราะไฟล์นี้เปลี่ยนทุกครั้งที่เปิด vault

## เครื่องมือ Claude Code เฉพาะของโปรเจกต์นี้

vault นี้มีชุด **custom agent** (`.claude/agents/`) และ **skill** (`.claude/skills/`) ที่ดูแลความสอดคล้องของ pipeline ข้างต้นโดยเฉพาะ เช่น แปลง requirement ดิบเป็น spec, แตก backlog เป็น feature list/user journey, สร้าง prototype, และสร้างเอกสารชุดทดสอบ (acceptance criteria/test plan/test case) — **ก่อนลงมือแก้ไขเอกสารในสาย pipeline นี้เอง ให้ตรวจสอบก่อนว่ามี skill ที่ตรงกับงานนั้นอยู่แล้วหรือไม่** แล้วใช้ skill นั้นแทนการแก้ไฟล์ตรงๆ เพื่อให้ธรรมเนียมของ vault (ภาษาไทย, wikilink, ห้ามลบ, การอัปเดต log/index) ถูกปฏิบัติตามอย่างสม่ำเสมอ
