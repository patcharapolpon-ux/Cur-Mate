...
**ข้อยกเว้น**: โฟลเดอร์ `homework/` เก็บโค้ดแบบฝึกหัดของคอร์สที่เรียนอยู่ (เช่น `homework/curmate-mvp/` และ `homework/curmate-mvp-v2/` เป็น static HTML/CSS/JS + Firebase สำหรับการบ้านเรื่อง database ต่อยอดกันเป็นรายสัปดาห์ — ดูรายละเอียดความต่างของแต่ละเวอร์ชันใน `SCOPE.md` ของแต่ละโฟลเดอร์) ไม่ใช่ส่วนหนึ่งของ pipeline เอกสารด้านล่าง และไม่ใช่แอปพลิเคชัน Curmate จริง — อยู่แยกจาก `docs/` โดยเจตนา ไม่ต้องพยายามเชื่อมโยงเข้ากับ pipeline หรือให้ agent/skill ของ vault เข้าไปยุ่งเกี่ยว
