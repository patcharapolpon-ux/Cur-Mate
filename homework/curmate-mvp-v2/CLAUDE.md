...
| `06-create-criteria-set.html` | `js/create-criteria-set.js` | เขียนเอกสารใหม่ลง `criteriaSets` (`status: pending` + `sourceFileUrl` ลิงก์ Google Drive ไม่บังคับ + `sourceMarkdownText`), เรียก AI สกัดกฎเกณฑ์จริงแล้วเขียน `rules` ใหม่หลายเอกสาร, เพิ่มแถว `criteriaSets/{id}/extractionLog` |
...
- **ไม่ได้ใช้ Firebase Storage ในโปรเจกต์นี้** (เคยเพิ่มไว้ช่วงสั้นๆ ในสัปดาห์ที่ 8 แล้วถอดออกภายในสัปดาห์เดียวกัน เพราะต้องใช้แผน Blaze — เปลี่ยนไปใช้ลิงก์ Google Drive แทน ดู [SCOPE.md](./SCOPE.md) หัวข้อ "เอกสารต้นฉบับ: เปลี่ยนจาก Firebase Storage เป็นลิงก์ Google Drive") ไม่ต้องเปิดใช้งาน Storage ที่ Firebase Console อีกต่อไป
