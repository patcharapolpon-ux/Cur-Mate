...
- `criteriaSets.status`: `"pending"` (ยังไม่เปิดใช้งาน, ซ่อนจากผู้ใช้ทั่วไป) ↔ `"active"` (เปิดใช้งานแล้ว) — **ไม่ได้คำนวณอัตโนมัติจากโค้ด** แม้กฎเกณฑ์ในชุดจะอนุมัติครบแล้วก็ตาม (ของ Sprint 2)
- `rules.status`: `"pending"` (รอตรวจสอบ) ↔ `"approved"` (อนุมัติแล้ว) — **ไม่มีสถานะ "rejected"** เพราะ spec ต้นทางยังไม่กำหนดพฤติกรรมการปฏิเสธไว้ ปุ่ม "ปฏิเสธ" ในหน้า 07 จึงเป็นแค่ mock alert ไม่เขียนอะไรกลับ Firestore
