...
flowchart TD
    ...
    D --> B
    C -->|ได้| E0[ตรวจสอบความสมบูรณ์ของเอกสารเทียบสารบัญ TOC และแจ้งเตือนถ้าไม่ครบ]
    E0 --> E2[สกัดข้อมูลรายวิชาและให้ผู้ใช้ตรวจสอบ/ยืนยันความถูกต้อง]
...
3. **ดึงข้อความจากไฟล์ได้หรือไม่ (เงื่อนไขแตกแขนง)** — ระบบตรวจสอบว่าสามารถดึงข้อความจากไฟล์ที่อัปโหลดได้หรือไม่ (อ้างอิง: [[../../01-requirements/01-spec/20260812-01-tqf2-consistency-check|ระบบตรวจสอบความสอดคล้องของเอกสารรายละเอียดหลักสูตร (มคอ.2)]] FR ข้อ 1)
   ...
   - **ได้** → ไปขั้นตอน "ตรวจสอบความสมบูรณ์ของเอกสารเทียบสารบัญ (TOC) และแจ้งเตือนถ้าไม่ครบ" หากเป็นการอัปโหลดไฟล์ซ้ำของเล่มเดิม ระบบจะแทนที่ไฟล์เดิมทันที (ไม่เก็บ version history)
...
5. **ตรวจสอบความสมบูรณ์ของเอกสารเทียบสารบัญ (TOC) และแจ้งเตือนถ้าไม่ครบ** — ต่อยอดจากขั้นตอนดึงข้อความสำเร็จ ระบบตรวจสอบเพิ่มเติมว่าโครงสร้างเนื้อหาของเอกสารครบตามที่สารบัญ (TOC) ของเล่มนั้นประกาศไว้หรือไม่ (เช่น TOC ระบุว่ามีภาคผนวก/หมวดหนึ่ง แต่เนื้อหาจริงในไฟล์ไม่มี) หากพบว่าไม่ครบ ระบบจะแจ้งเตือนผู้ใช้ว่าเอกสารอาจไม่สมบูรณ์ก่อนดำเนินการตรวจสอบต่อ — ขั้นตอนนี้**ไม่ปฏิเสธไฟล์** ต่างจากกรณีดึงข้อความไม่ได้ในขั้นตอนก่อนหน้า เพราะเอกสารยังตรวจสอบส่วนที่มีอยู่ได้ เป็นเพียงการแจ้งเตือนแบบไม่บล็อก flow (ทำงานเสมอไม่ว่าเอกสารจะครบหรือไม่ครบ แล้วดำเนินการต่อไปยังขั้นตอนสกัดข้อมูลรายวิชา) (อ้างอิง: [[../../01-requirements/01-spec/20260812-01-tqf2-consistency-check|ระบบตรวจสอบความสอดคล้องของเอกสารรายละเอียดหลักสูตร (มคอ.2)]] FR ข้อ 6)
6. **สกัดข้อมูลรายวิชาและให้ผู้ใช้ตรวจสอบ/ยืนยันความถูกต้อง** — ระบบทำงานแบบกึ่งอัตโนมัติ โดยสกัดข้อมูลรายวิชา (รหัสวิชา ชื่อรายวิชา หน่วยกิต คำอธิบายรายวิชา) จากทุกจุดที่ปรากฏซ้ำในเล่ม แล้วแสดงให้ผู้จัดทำหลักสูตรตรวจสอบ ยืนยัน หรือแก้ไขจุดที่ระบบดึงข้อมูลมาไม่มั่นใจ ก่อนไปขั้นตอนเลือกชุดเกณฑ์ (อ้างอิง: [[../../01-requirements/01-spec/20260812-01-tqf2-consistency-check|ระบบตรวจสอบความสอดคล้องของเอกสารรายละเอียดหลักสูตร (มคอ.2)]] FR ข้อ 2)
7. **เลือกชุดเกณฑ์ criteria set** — ผู้ใช้เลือกชุดเกณฑ์ที่ต้องการอ้างอิงตอนตรวจสอบ เลือกได้มากกว่า 1 ชุดพร้อมกัน (multi-select) จากรายชื่อชุดเกณฑ์ที่มีสถานะ "เปิดใช้งาน (active)" เท่านั้น (อ้างอิง: [[../../01-requirements/01-spec/20260812-01-tqf2-consistency-check|ระบบตรวจสอบความสอดคล้องของเอกสารรายละเอียดหลักสูตร (มคอ.2)]] FR ข้อ 3)
8. **เลือกอย่างน้อย 1 ชุดแล้วหรือยัง (เงื่อนไขแตกแขนง)** — ระบบบังคับตรวจสอบว่าเลือกชุดเกณฑ์อย่างน้อย 1 ชุดแล้วหรือไม่ ปุ่มเริ่มตรวจสอบจะกดไม่ได้ถ้ายังไม่เลือก (อ้างอิง: [[../../01-requirements/01-spec/20260812-01-tqf2-consistency-check|ระบบตรวจสอบความสอดคล้องของเอกสารรายละเอียดหลักสูตร (มคอ.2)]] FR ข้อ 3)
   - **ยัง** → วนกลับไปขั้นตอน "เลือกชุดเกณฑ์ criteria set"
   - **เลือกแล้ว** → ไปขั้นตอน "กดเริ่มตรวจสอบ"
9. **กดเริ่มตรวจสอบ** — ผู้ใช้กดปุ่มเริ่มตรวจสอบเพื่อให้ระบบเริ่มประมวลผลเปรียบเทียบเอกสารกับชุดเกณฑ์ที่เลือกไว้ (อ้างอิง: [[../../01-requirements/01-spec/20260812-01-tqf2-consistency-check|ระบบตรวจสอบความสอดคล้องของเอกสารรายละเอียดหลักสูตร (มคอ.2)]] FR ข้อ 3)
10. **ตรวจสอบความสอดคล้องภายในเล่มเดียวกัน** — ระบบตรวจว่ารหัสวิชา ชื่อวิชา หน่วยกิต และคำอธิบายรายวิชาที่ปรากฏซ้ำในหลายจุดของเล่มเดียวกันตรงกันทุกจุด รวมถึงตรวจการอ้างอิงเกณฑ์มาตรฐานให้ตรงกับชุดเกณฑ์ที่เลือกไว้ (อ้างอิง: [[../../01-requirements/01-spec/20260812-01-tqf2-consistency-check|ระบบตรวจสอบความสอดคล้องของเอกสารรายละเอียดหลักสูตร (มคอ.2)]] FR ข้อ 2 และ FR ข้อ 4 ส่วนภายในเล่ม)
11. **ตรวจสอบความสอดคล้อง/ซ้ำซ้อนข้ามเล่มหลักสูตร** — ระบบเปรียบเทียบรายวิชาระหว่างเล่มหลักสูตรที่อยู่ในระบบ เพื่อตรวจจับรหัสวิชาซ้ำซ้อนโดยไม่ตั้งใจ และวิชาแกนที่น่าจะเป็นวิชาเดียวกันแต่ถูกป้อนรหัส/ชื่อไม่ตรงกันระหว่างเล่ม (อ้างอิง: [[../../01-requirements/01-spec/20260812-01-tqf2-consistency-check|ระบบตรวจสอบความสอดคล้องของเอกสารรายละเอียดหลักสูตร (มคอ.2)]] FR ข้อ 4 ส่วนข้ามเล่ม)
12. **แสดงรายงานสรุปผลตรวจสอบ** — ระบบแสดงรายงานสรุปจุดที่ไม่สอดคล้องกันเป็นรายการ (list) พร้อมระบุตำแหน่ง/หน้าที่พบในเอกสารสำหรับแต่ละจุด (อ้างอิง: [[../../01-requirements/01-spec/20260812-01-tqf2-consistency-check|ระบบตรวจสอบความสอดคล้องของเอกสารรายละเอียดหลักสูตร (มคอ.2)]] FR ข้อ 5)
13. **ทบทวนรายงานและนำไปแก้ไข/ส่งอนุมัติ** — ผู้จัดทำหลักสูตรทบทวนรายงานและนำไปแก้ไขเอกสารก่อนอัปโหลดตรวจสอบซ้ำ หรือส่งเข้าสู่กระบวนการอนุมัติ (ไม่มี requirement เฉพาะ — เป็นจุดจบของ flow)
...
| ตรวจสอบความสมบูรณ์ของเอกสารเทียบสารบัญ (TOC) และแจ้งเตือนถ้าไม่ครบ | [[../../01-requirements/01-spec/20260812-01-tqf2-consistency-check\|ระบบตรวจสอบความสอดคล้องของเอกสารรายละเอียดหลักสูตร (มคอ.2)]] (FR ข้อ 6) |
